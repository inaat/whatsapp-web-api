    ...
    sock?.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            if (status === DisconnectReason.loggedOut || status === 405 || status === 402 || status === 403) {
                ...
                return;
            } else if (status === DisconnectReason.connectionClosed || status === DisconnectReason.connectionLost) {
                await delay(3000);
...
sock?.ev.on('presence.update', async (json) => {
    ...
    sock?.ev.on('messages.upsert', async (m) => {
        ...
        m.messages.map(async (msg) => {
			...
			if (this.instance.base64 === true) {
                ...
                switch (messageType) {
                    ...
                    case 'videoMessage':
                        ...
                        //webhookData['msgContent'] = await fs.readFile(video_file, {
                            //encoding: 'base64',
                        //});

                        //webhookData['thumb'] = await this.thumbBase64(video_file);
			...
			const audio_file = await downloadMessage(msg.message.audioMessage,'audio');				
			const buffer = Buffer.from(audio_file, 'base64');
...
getWhatsappCode(id)
 ...
 if (id.startsWith('55')) {
        ...
        if (comprimentoSemDDD < 8) {
            throw new Error('No WhatsApp account exists!');
...
{
	console.log('Group processing '+group+' started')
    ...
    if (origem === 'url') {
        ...
        if (parsedUrl.protocol === 'http:' || parsedUrl.protocol === 'https:') {
            ...
            if (!myArray.includes(mimetype.trim())) {
                throw new Error('File type ' + mimetype + ' is not allowed for ' + data.type);
            }
...
async GetFileMime(file) {
    try {
        const fileInfo = await await axios.head(file);
        return fileInfo.headers['content-type'];
        return file;
    } catch (error) {
        throw new Error(
            'Invalid file'
...
async sendMedia(to, userType, file, type, caption = '', replyFrom = false, d = false) {
    ...
    if (!myArray.includes(mime.trim())) {
        throw new Error('File type ' + mime + ' is not allowed for ' + type);
    }
